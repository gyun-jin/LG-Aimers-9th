...
- `batter_id_mapping_clean.csv`
  - hand cleaning analysis에서 만든 batter mapping 파일. v5-3의 직접 Trackman inference feature보다는 hand 정제/분석 재현 문맥에 사용한다.
...
`trackman_history.csv`는 공식 원본 대용량 데이터라 Git에 포함하지 않는다. 재학습할 때는 공식 제공 파일을 프로젝트의 `data/trackman_history.csv` 또는 원래 작업 경로의 `train_trackman/trackman_history.csv`에 별도로 배치해야 한다.
...
- 학습 때는 `pitcher_id_mapping_clean.csv`와 별도 배치한 공식 `trackman_history.csv`가 필요하다.
