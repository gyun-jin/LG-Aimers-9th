...
| 예측 범위 | 통과 | sample 기준 `[0.40797738, 0.49590037]` |
| NaN 없음 | 통과 | sample 실행 기준 NaN `0` |
| 외부 다운로드/인터넷 | 통과 | 없음 |
| test 내부 집계/정답값 누출 | 통과 | 없음 |
| Trackman 사용 | 조건부 통과 | 제공 보조 데이터 기반 이전 시즌 요약값을 학습 번들에 저장해서 사용 |
...
형식 오류 가능성은 낮다. 다만 서버 점수 관점에서는 `/5`보다 확실히 좋다고 보기 어렵다. 평균 OOF Brier는 좋아졌지만 최악 fold는 거의 동률이다.
