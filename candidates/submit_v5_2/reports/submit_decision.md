...
다만 `/5` 서버 제출 점수 `938.2526739958`을 이미 확인한 상태라면, `5-2`는 평균 Brier 개선을 노린 대체 후보이지 확실한 상위 제출본은 아니다. 전체 fold 기준으로 평균 Brier는 좋아졌지만 최악 fold는 거의 동률이다.
...
- 테스트 실행 결과: 행 수 `5`, 컬럼 `row_id`, `control_success`, NaN `0`, 예측 범위 `[0.40797738, 0.49590037]`
