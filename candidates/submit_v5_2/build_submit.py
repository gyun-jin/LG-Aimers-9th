...
def build_oof_predictions(train: pd.DataFrame, y: np.ndarray, schema: dict[str, Any], tm_state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for valid_season in FOLD_SEASONS:
        ...
        state = task5.fit_feature_state(train.iloc[train_idx], y[train_idx], schema["alpha"])
        base_features = task5.build_v3_features(train, state)
        x_all, cats = task52.prepare_features(base_features, tm_features, SELECTED_FEATURE_SET)
...
def fit_final_models(train: pd.DataFrame, y: np.ndarray, schema: dict[str, Any], tm_state: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str], list[str]]:
    state = task5.fit_feature_state(train, y, schema["alpha"])
    base_features = task5.build_v3_features(train, state)
    tm_features = task51.build_trackman_features(train, tm_state, TRACKMAN_STRATEGY)
    x_train, cats = task52.prepare_features(base_features, tm_features, SELECTED_FEATURE_SET)
    components = []
...
def main() -> None:
    ...
    write_json(
        MODEL_DIR / "ensemble_config.json",
        {
            ...
            "trackman_strategy": TRACKMAN_STRATEGY,
            "selection_metric": "full three-fold mean Brier; packaged by user request despite near-tie worst fold caveat",
        },
    )
    shutil.copyfile(TM_ROOT / "script.py", ROOT / "script.py")
